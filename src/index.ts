...
async function main() {
  ...
  const { dependencies, devDependencies } = await checkDependencies(resolvedDir); // Pass resolved directory to checkDependencies
  ...
  const unusedDeps = getAllDeps(dependencies, devDependencies).all.filter(dep => !allImports.includes(dep));
